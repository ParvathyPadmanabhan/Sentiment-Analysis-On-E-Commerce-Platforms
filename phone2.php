...
$sql="SELECT * from phone where model_name='$_REQUEST[model]' limit 0,1";
...
<td><?PHP echo "<a href='?model=$row[model_name]'>".$row['model_name']."</a>";?></td>
...
$sql1="SELECT AVG(score) as average_score
      FROM phone where model_name='$_REQUEST[model]'";
...
$sql="SELECT * from phone where model_name='$_REQUEST[model]' ";
$result=$conn->query($sql)or die(mysql_error()); 
while($row = mysqli_fetch_assoc($result)) 
{ 
?>
...
<form action = "<?php echo $_SERVER['PHP_SELF']; ?>" method='post'>
    ...
    <input type = 'hidden' id = 'sentence' value='phone' name='field' />
